...
Agent.prototype = {
  ...
  _connectToMongoDatabase: function(name, connectionData) {
    logger.info(
      `Agent '${this.id}': connecting to a Mongo service '${name}'`,
      connectionData
    );
  ...
  _removeFrames: function(directoryName) {
    let configuration =
      this.configuration;
    let programConfigurations =
      configuration["programs"];
    ...
    let frameDirectory =
      path.join(workingDirectory, directoryName);

    try {
      if (fs.statSync(frameDirectory).isDirectory()) {
        let entries =
          fs.readdirSync(frameDirectory);

        entries =
          entries.map(entry => path.join(frameDirectory, entry));
        entries =
      ...
      logger.info(
        `Failed to cleanup the frames directory '${directoryName}'`
      );
    }
  },

  _loadFrames: function(directoryName) {
    let frames =
      [];
    ...
    try {
      if (fs.statSync(frameDirectory).isDirectory()) {
        ...
        frames =
          entries.map(entry => fs.readFileSync(entry));
      }
    } catch (error) {
      logger.info(
        `Failed to load frames from the directory '${directoryName}'`
      );
    }
  ...
  _startNeuralDoodleSubprocess: function(task, styleFile, outputSemanticMapFile) {
    logger.info(
      `Agent '${this.id}': starting a neural-doodle subprocess ` +
      `to handle the task '${task.id}'`
    );
  ...
  _saveTask: function(task) {
    logger.info(`Agent '${this.id}': saving the task '${task.id}'`);
